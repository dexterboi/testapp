...
const UserBookingsPage = () => {
    ...
    const handleConfirmCancel = async () => {
        ...
        try {
            ...
            setIsCancelling(false);
            setErrorMessage(t('common.error_occurred'));
    ...
    const getStatusBadge = (booking: any) => {
        ...
        if (isPast && booking.status === 'approved') {
            return <span className="px-3 py-1 bg-app-surface text-app-text-muted rounded-lg text-[10px] font-black uppercase tracking-widest border border-app-border">{t('activity.completed')}</span>;
        }
        switch (booking.status) {
            case 'pending': return <span className="px-3 py-1 bg-amber-500/10 text-amber-500 rounded-lg text-[10px] font-black uppercase tracking-widest border border-amber-500/20 italic">{t('activity.pending')}</span>;
            case 'approved': return <span className="px-3 py-1 bg-primary/10 text-primary rounded-lg text-[10px] font-black uppercase tracking-widest border border-primary/20 shadow-sm shadow-primary/5">{t('activity.confirmed')}</span>;
            case 'cancel_request': return <span className="px-3 py-1 bg-orange-500/10 text-orange-500 rounded-lg text-[10px] font-black uppercase tracking-widest border border-orange-500/20 italic">{t('activity.cancelling')}</span>;
            default: return <span className="px-3 py-1 bg-app-surface text-app-text-muted rounded-lg text-[10px] font-black uppercase tracking-widest border border-app-border">{booking.status?.toUpperCase() || t('activity.unknown')}</span>;
        }
    };
        ...
        <div className="min-h-screen bg-app-bg pb-[calc(8rem+env(safe-area-inset-bottom))] text-app-text transition-colors duration-300">
            <header className="px-6 pt-[calc(3rem+env(safe-area-inset-top))] pb-6 sticky top-0 z-40 bg-app-bg/80 backdrop-blur-xl border-b border-app-border">
                <div className="flex justify-between items-center mb-6">
                    <div>
                        <p className="text-app-text-muted text-[10px] font-black uppercase tracking-[0.3em] mb-1 opacity-60">{t('activity.hub')}</p>
                        <h1 className="text-3xl font-black tracking-tighter text-app-text leading-none">{t('activity.my_matches.split')[0] || 'My'} <span className="text-primary italic">{t('activity.my_matches.split')[1] || 'Matches'}</span></h1>
                    </div>
                ...
                <div className="flex gap-2 p-1 bg-app-surface-2 rounded-[1.2rem] border border-app-border overflow-x-auto no-scrollbar">
                    {[
                        { key: 'upcoming', label: t('activity.active'), icon: 'bolt' },
                        { key: 'past', label: t('activity.history'), icon: 'history' },
                        { key: 'cancelled', label: t('activity.others'), icon: 'close' }
                    ].map(({ key, label, icon }) => (
                        <button
                            key={key}
            ...
            <div className="px-6 mt-4 space-y-4 animate-in fade-in slide-in-from-bottom-8 duration-700">
                {loading ? (
                    <div className="bg-app-surface p-16 rounded-[2.5rem] flex flex-col items-center justify-center border border-app-border">
                        ...
                        <p className="text-[10px] font-black text-app-text-muted uppercase tracking-[0.3em] animate-pulse">{t('social.loading')}</p>
                    </div>
                    ...
                    <div className="bg-app-surface p-12 rounded-[2.5rem] text-center border border-app-border">
                        ...
                        <p className="text-app-text font-black text-lg tracking-tight uppercase mb-4">{t('activity.no_matches')}</p>
                        <button onClick={() => navigate('/')} className="bg-app-surface-2 hover:bg-app-surface-2/80 text-primary px-6 py-3 rounded-full font-black text-[10px] uppercase tracking-widest transition-colors border border-app-border">{t('activity.find_pitch')}</button>
                    </div>
                ) : (
            ...
            {cancellingBooking && (
                <ConfirmationModal
                    isOpen={true}
                    title={t('activity.cancel_match')}
                    message={t('activity.cancel_confirm')}
                    confirmText={isCancelling ? t('pitch.processing') : t('common.confirm')}
                    onConfirm={handleConfirmCancel}
                    onCancel={() => setCancellingBooking(null)}
                    type="warning"
                />
            )}
            <SuccessModal isOpen={showSuccessModal} title={t('activity.request_sent')} message={t('activity.request_sent_sub')} onClose={() => setShowSuccessModal(false)} />

...
const BookingConfirmPage = () => {
    ...
    if (isPaid) {
        return (
            <div className="min-h-screen bg-slate-950 flex flex-col items-center justify-center text-white p-8 text-center overflow-auto pb-20">
                ...
                <div className="bg-slate-900/50 backdrop-blur-xl text-white p-8 rounded-[2.5rem] w-full max-w-sm shadow-2xl mb-10 border border-app-border relative overflow-hidden">
                    ...
                    <div className="bg-app-surface-2 p-4 rounded-2xl mb-6 border border-app-border text-white">
                ...
                <button onClick={() => navigate('/')} className="w-full bg-app-surface-2 text-primary py-4 rounded-[2rem] font-black shadow-xl active:scale-95 transition-all text-xs tracking-widest uppercase border border-app-border hover:bg-app-surface-2">
                    BACK TO DASHBOARD
                </button>
            </div>
        );
    ...
    return (
        <div className="min-h-screen bg-slate-950 p-6 flex flex-col pb-safe text-white">
            <div className="flex items-center justify-between mb-8 pt-6">
                <button onClick={() => navigate(-1)} className="bg-slate-900/50 backdrop-blur-md p-3 rounded-2xl shadow-sm border border-app-border text-white"><ChevronLeft size={20} /></button>
                <h1 className="text-xs font-black tracking-[0.2em] text-slate-500 uppercase">Checkout</h1>
                <div className="w-10"></div>
            </div>

            <div className="bg-slate-900/50 backdrop-blur-md p-6 rounded-[2rem] shadow-sm border border-app-border mb-6">
                ...
                <div className="flex gap-4 mb-6 p-3 bg-app-surface-2 rounded-2xl border border-app-border">
                    ...
                    <div>
                        ...
                        <p className="text-[10px] text-slate-500 font-black uppercase tracking-widest">{state.date} • {state.time}</p>
                    </div>
                </div>
                <div className="space-y-4">
                    <div className="flex justify-between text-xs font-bold text-slate-400">
                        <span className="uppercase tracking-widest opacity-60">Pitch Fee</span>
                        <span className="text-white font-black">TND {state.pitch.price_per_hour || state.pitch.pricePerHour || 0}</span>
                    </div>
                    <div className="flex justify-between text-xs font-bold text-slate-400">
                        <span className="uppercase tracking-widest opacity-60">Service Fee</span>
                        <span className="text-white font-black">TND 2.00</span>
                    </div>
                ...
                <div className="mt-6 pt-6 border-t border-app-border">
                    <div className="flex items-center gap-3 bg-app-surface-2 p-4 rounded-2xl border border-app-border">
                        ...
                        <div>
                            <p className="text-[10px] font-black text-slate-500 uppercase tracking-widest leading-none mb-1">Contact Verification</p>
                            <p className="text-xs font-black text-white uppercase tracking-tighter">Phone sync active</p>
                        </div>
                    </div>
                </div>
            </div>

            <div className="bg-slate-900/50 backdrop-blur-md p-6 rounded-[2rem] shadow-sm border border-app-border mb-auto">
                <div className="flex items-center justify-between mb-4">
                    <h3 className="font-black text-white tracking-tight uppercase tracking-widest text-[10px]">PAYSPLIT™</h3>
                ...
                <div className="flex gap-3">
                    <button className="flex-1 py-3 border-2 border-app-border rounded-2xl text-[10px] font-black text-slate-500 tracking-widest uppercase hover:border-primary/20 transition-all">SPLIT (1/10)</button>
